...
export const createNotification = async (receiver, type, message = '') => {
    try {
        let notification;
        const currentUser = await getCurrentUser();
        notification = createNotificationArr(receiver, type, message)
...
const createNotificationArr = (receiver, type, message = '') => {
    let notification = {};
    ...
    switch (type) {
        ...
        case 'Friend Request':
            ...
            notification.description = `${senderName} sent you a friend request`
            break;
    }
    return notification;
}
