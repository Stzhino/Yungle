...
export const getConnections = async () => {
    try {
        ...
        // query for whether the current user has sent an outgoing req or has rejected the suggestion or if they're friends with the user, check if the current user is the target of a friend req and has accepted that request
        ...
        const connections = await databases.listDocuments(databaseId, connectionsId, [Query.or([Query.and(
            [Query.equal('senderId', currentAccount.$id), Query.or([Query.equal('outgoingRequest', true), Query.equal('suggestionRejected', true), Query.equal('friendReqAccepted', true)])]),
        Query.and([
            Query.equal('receiverId', currentAccount.$id), Query.equal('friendReqAccepted', true)
        ])])]);
...
export const createChatSession = async (personAId, personBId, personAName, personBName) => {
    try {
        if (!personAId || !personBId || !personAName || !personBName) {
            throw new Error("Missing required parameters for chat session.");
        }

        const chatTitle = `${personAName.trim()} & ${personBName.trim()}`;
...
export async function getRecommendationsAll() {
    let userCollection = await getUsers();
    // console.log(userCollection)
    let connectionCollection = await getConnections();
    // console.log(connectionCollection)
    let connectionUserIds = new Set(connectionCollection.flatMap(connection => [connection.receiver.$id, connection.sender.$id]).filter(Boolean));
    let filteredUsers = userCollection.filter(user => !connectionUserIds.has(user.$id));
    return filteredUsers;
}

// takes in an interest or all
export async function getRecommendations(category) {
    console.log("reached")
    let userCollection = await getInterest(category);
    // console.log(userCollection)
    let connectionCollection = await getConnections();
    // console.log(connectionCollection);
    let connectionUserIds = new Set(connectionCollection.flatMap(connection => [connection.receiver.$id, connection.sender.$id]).filter(Boolean));
...
export const createMessage = async (sessionID, messageText) => {
    try {
...
export const getNotification = async () => {
    try {
        ...
        return notification.documents.reverse();
    } catch (error) {
        throw new Error(error)
    }
}
...
export async function updateNotif(sender, wasAccepted) {
    try {
        print("reached")
        const currentUser = await getCurrentUser();
        const notifications = await databases.listDocuments(
            databaseId,
            notificationId,
            [Query.equal('sender', sender), Query.equal('receiver', currentUser.$id), Query.equal('title', 'Friend Request')]
        )
...
export async function getChatSession() {
    try {
        ...
        const chatSession = await databases.listDocuments(
            databaseId,
            chatSessionId,
            [
                Query.or([
        ...
        console.log("Chat sessions fetched:", chatSession.documents);
...
export const deleteChatSession = async (sessionId) => {
    try {
        ...
        await Promise.all(messages.documents.map(message => 
            databases.deleteDocument(databaseId, messageId, message.$id)
        ));

        // Then delete the chat session itself
        await databases.deleteDocument(databaseId, chatSessionId, sessionId);
...
export async function fetchFriends(){
    try {
        ...
        if (!currentAccount) throw Error;
        console.log(currentAccount.$id);
        // want to query for anyone who is friends with the user

        const friendList = await databases.listDocuments(databaseId, connectionsId, [Query.or([Query.equal('senderId', currentAccount.$id),Query.equal('receiverId', currentAccount.$id)]), Query.equal('friendReqAccepted', true)]);

        if (!friendList || friendList.documents.length === 0) {
            console.log("No friends found");
            return [];
        }
        
        let userCollection = await getUsers();
        let setFriends = new Set(friendList.documents.map(connection => [connection.receiver?.$id, connection.sender?.$id]).flat().filter(Boolean));
        // console.log(setFriends)
        let filteredUsers = userCollection.filter(user => setFriends.has(user.$id));
        return filteredUsers;
    }
    catch (error) {
        throw new Error(error);
