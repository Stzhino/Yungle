...
export const getInterest = async (query) => {
    try{
        // const currentAccount = await account.get();
        // if(!currentAccount) throw Error;
        // const users = await databases.listDocuments(databaseId, userCollectionId, [Query.notEqual('accountId',currentAccount.$id) && Query.equal('interest',query)]);
...
export const createNotification = async(sender,profileuri, type, message='')=>{
    try{
        let notification;
        notification=createNotificationArr(sender,profileuri, type, message)
        const currentUser = await getCurrentUser();
        notification.receiver=currentUser.$id;
...
const createNotificationArr=(sender,profileuri, type, message='')=>{
    let notification={};
    notification.name=sender;
    notification.profileuri=profileuri;
    notification.time = new Date().toISOString();
    switch(type){
        case 'Message':
        notification.title=`Message from ${sender}`
        notification.description=`${sender} commented:"${message}"`
        break;
        case 'Like':
            notification.title=`New Like`
            notification.description=`${sender} liked your post`
        break;
    }
    return notification;
}